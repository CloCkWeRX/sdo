...
require_once 'SCA/SCA_CommentReader.php';
...
class SCA_AnnotationReader
{
     ...
     *
     * @param mixed $component_instance_or_class_name Class to annotate
     */
                ...
                if ($reader->isReference()) {
                    ...
                    if ($ref == "") {
                        throw new SCA_RuntimeException(
                            "Instance variable " . $ref_prop->getName()
                            . " has a binding annotation with no following value"
                        );
                    }
                    ...
                    if ($reader->hasBinding()) {
                        throw new SCA_RuntimeException(
                            "Instance variable " . $ref_prop->getName()
                            . " appears to have a @binding but no @reference"
                        );
                    }
                }
            }
    ...
    public function reflectReferencesFull()
    {
        ...
        try {
            ...
            throw new SCA_RuntimeException(
                "The following error occured while examining the"
                . " comment block for instance variable "
                . $ref_prop->getName() . ": " .
                $e->getMessage()
            );
        }
    ...
    private function _getReflection()
    {
        if ($this->instance_of_the_component !== null) {
            return new ReflectionObject($this->instance_of_the_component);
        }

        return new ReflectionClass($this->class_name);
    }
    ...
    public function reflectService()
    {
        ...
        if (!$reader->isService()) {
            throw new SCA_RuntimeException(
                "Class $sca_name does not contain an @service annotation."
            );
        }
        ...
        if ($reader->isService()) {
            ...
            $interface_reflection = $this->_getInterfaceReflection(
                $reflection,
            ...
            if (is_null($interface_reflection)) {
                ...
                if (!empty($service->interface_name)) {
                    throw new SCA_RuntimeException(
                        "Service interface "
                        . $service->interface_name
                        . " specified by @service does not match any interface"
                        . " implemented by {$sca_name}.php'."
                    );
                }
        ...
        if (count($service->binding) == 0) {
            throw new SCA_RuntimeException(
                "No valid @binding annotation could be found for '{$sca_name}.php'."
            );
        }
        ...
        $service_methods = SCA_Helper::filterMethods(
            $this->_getMethods(),
        ...
        */
        foreach ($service_methods as $service_method) {
            ...
            if ($comment) {
                ...
                $methodAnnotations = $method_reader->getMethodAnnotations();
                ...
                foreach ($methodAnnotations as $annotationSet) {
                    ...
                    foreach ($annotationSet as $annotation) {
                        $is_param = strcmp(
                            ...
                            SCA_AnnotationRules::PARAM
                        );

                        if ($is_param === 0) {
                            $has_dollar = strpos(
                                $annotation['name'],
                                SCA_AnnotationRules::DOLLAR
                            );
                            ...
                            if ($has_dollar === false) {
                                throw new SCA_RuntimeException(
                                    "Invalid syntax '"
                                    . $annotation['name']
                                    . "' is not a php variable name"
                                );
                            }

                            $methodAnnotations['parameters'][$thisElement]['name'] = trim(
                                $annotation['name'],
                                SCA_AnnotationRules::DOLLAR
                            );
                        }
                        ...
                        if (array_key_exists('namespace', $annotation)) {
                            ...
                            if (!$matched_xsds) {
                                /*
                                TODO: noticed potential defect that if a method
                                A has no @param, the @param of another method is
                                being picked up instead and this error being thrown
                                for method A
                                */
