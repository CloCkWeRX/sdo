...
if ( ! class_exists('SCA_GenerateWsdl', false) ) {
    class SCA_GenerateWsdl
    {
        ...
        public static function generateDocumentLiteralWrappedWsdl($service_desc)
        {
            ...
            ***********************************************************************/
            $types_pos = strpos($str, '<types/>');
            $front     = substr($str, 0, $types_pos);     // the bit before <types/>
            $back      = substr($str, $types_pos + 8);    // the bit after <types/>
