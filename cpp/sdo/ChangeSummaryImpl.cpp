...
namespace sdo{
		...
		DataObjectImpl* container, const Property& prop)
	{
		...
		for (i=0; i < pl.size(); i++) 
		{
			...
			if (pl[i].isMany())
			{
				...
				for (int j=0;j< dol.size(); j++)
				{
					...
					if (pl[i].getType().isDataType()) {
					    ...
					    sl.append(Setting(true,
							false, value,len,pl[i],j));
					}
					else {
						value = (void*)dol[j];
						sl.append(Setting(true,
							false, value,0,pl[i],j));
					}
				}
			}
			else 
			{
				setPropValue(&value,&len,ob,pl[i]);
				sl.append(Setting(true,
					ob->isNull(pl[i]), value,len,pl[i],0));
			}
		 ...
		 const Property& prop)
	{
		...
		else 
		{
			...
			slist.append(Setting(true,
				ob->isNull(prop),value,len,prop,0));
		}
			...
			theType(dob->getType())
	{
		...
		const char* path = ((DataObjectImpl*)dob)->objectToXPath();
		if (path !=0  && strlen(path) != 0) 
		{
			theOldXpath = new char[strlen(path)+1];
			strcpy(theOldXpath,path);
		}
		else 
		{
			theOldXpath = 0;
	...
	const char* ChangeSummaryImpl::deleteLogItem::getOldXpath()
	{
		return theOldXpath;
	}
