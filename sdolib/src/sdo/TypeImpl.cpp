...
//TODO set these to valid sizes, or alter the storage method
...
namespace sdo{
	...
	{
		Logger::log("ERROR TypeImpl copy constructor\n " );
		isResolving = false;
		isResolved = false;
	}

	 TypeImpl::TypeImpl(const Type* base, const char* uri, const char* inname, bool isSeq,
		 bool isOp) 
	 {
		init(uri,inname,isSeq,isOp);
		baseType = (TypeImpl*)base;
	 }

	 TypeImpl::TypeImpl(const char* uri, const char* inname, bool isSeq,
		 bool isOp) 
	 ...
	 void TypeImpl::init(const char* uri, const char* inname, bool isSeq,
		 bool isOp)
	 {
		isResolving = false;
		isResolved = false;
		...
		isSequenced = isSeq;
		isOpen = isOp;
 		Logger::log("TypeImpl Second constructor \n");
	...
	TypeImpl::TypeImpl()
	{
		Logger::log("TypeImpl void constructor \n");
	}
	...
	TypeImpl::~TypeImpl()
	{
		 Logger::log("TypeImpl destructor \n");
	...
	void TypeImpl::initCompoundProperties()
	{
		...
		if (isResolved)
		{
			//cout << "Type already resolved:" << getName() << endl;
			return;
		}

		isResolving = true;
		
		//cout << "Resolving type: " << getName() << endl;
