...
{	
	...
	try {
		if (xpath) {
			...
			my_object->dop->unset(propertyIndex);
		}
	} catch(SDORuntimeException e) {
		sdo_throw_runtimeexception(&e TSRMLS_CC);
	}
...
{	
	...
	switch(Z_TYPE_P(offset)) {
	case IS_NULL:
		/* only makes sense for open types */
		php_error(E_ERROR, "%s:%i: NULL dimension not yet implemented", CLASS_NAME, __LINE__);
		return;
	case IS_STRING:	
		xpath = Z_STRVAL_P(offset);
		xpath_len = Z_STRLEN_P(offset);
...
/*		
 *		zend_printf("WRITE: xpath=%s, propertyName=%s, isSet=%i, isMany=%i, isIndexed=%i, type=%i\n", 
 *			xpath, property.getName(), dop->isSet(xpath), property.isMany(), XpathHelper::isIndexed(xpath), property.getTypeEnum()); 
 */

		if (Z_TYPE_P(z_propertyValue) == IS_NULL) {
			/* TODO: fix this when the C++ lib supports a NULL property  */
			zend_throw_exception_ex(sdo_invalidconversionexception_class_entry, 0 TSRMLS_CC, 
				"can't assign NULL to %s", xpath);
		} else {
			/*
			...
			case Type::UriType:
				convert_to_string(&temp_zval);
				dop->setBytes(xpath, Z_STRVAL(temp_zval), 1 + Z_STRLEN(temp_zval));
				break;
			case Type::DataObjectType:
...
{	
	...
	try {
		return ((iterator->valid && iterator->index >= 0 && 
			iterator->index < my_object->dop->getInstanceProperties().size()) ? SUCCESS : FAILURE);
	} catch (SDORuntimeException e) {
...
static zval *sdo_do_populate_graph (DataObjectPtr dop TSRMLS_DC)
{
    ...
    for (int i = 0; i < pl.size(); i++) {
        if (dop->isSet(pl[i])) {
            ...
            } else if (pl[i].getType().isDataObjectType() && ! pl[i].isContainment()) {
...
static int sdo_do_unserialize (zval **object, zend_class_entry *ce, const unsigned char *buffer, zend_uint buffer_length, zend_unserialize_data *data TSRMLS_DC)
{
	...
	try {  
		...
		sdo_das_df_new(factory_zval, (DASDataFactory *)(DataFactory *)xmldasp->getDataFactory() TSRMLS_CC);
